...
def train_mle_estimator(train_images, train_labels):
    ...
    n_class = np.ndarray.sum(train_labels, axis=0) # Number of labels in each class
    theta_mle = np.dot(train_images.T, train_labels) / n_class # MLE estimate of theta
    pi_mle = n_class / train_images.shape[0] # MLE estimate of pi
...
def train_map_estimator(train_images, train_labels):
    ...
    n_class = np.ndarray.sum(train_labels, axis=0) # Number of labels in each class
    theta_map = (np.dot(train_images.T, train_labels) + 2) / (n_class + 4) # MAP estimate of theta
    pi_map = n_class / train_images.shape[0] # MAP estimate of pi
...
def predict(log_like):
    ...
    predictions = np.argmax(log_like, axis=1) # List of indices of max log-likehood for each image
    return predictions
